...
def array_to_str(arr): # arr:(x,), turn an arr to a sentence
    ...
    return out.strip()
'''
def get_self_critical_reward(model, feat, feat_mask, groundtruth, gen_result):  # gen_result:(m,seq_length), groundtruth is a list, elem of which is tensor
    batch_size = gen_result.size(0)
    # seq_per_img = batch_size // len(data['gts'])  # what the true meaning of seq_per_img?

    # get greedy decoding baseline
    greedy_res, _ = model.sample(Variable(feat.data, volatile=True), Variable(feat_mask.data, volatile=True)) # sample_max=1
    ...
    return rewards
'''
def get_self_critical_reward(model, feat1, feat2, feat3, feat4, feat_mask, pos_feat, groundtruth, gen_result):  # gen_result:(m,seq_length), groundtruth is a list, elem of which is tensor
    batch_size = gen_result.size(0)
    # seq_per_img = batch_size // len(data['gts'])  # what the true meaning of seq_per_img?
    ...
    greedy_res, _ = model.sample(Variable(feat1.data, volatile=True),
				 ...
				 Variable(feat3.data, volatile=True),
				 Variable(feat4.data, volatile=True),
                                 Variable(feat_mask.data, volatile=True),
				 Variable(pos_feat.data, volatile=True)) # sample_max=1
