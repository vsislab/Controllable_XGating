...
class EncoderLstm_two_fc_nogate(nn.Module):
	''' generally, take RGB and OF fc_features as input:
		...
		4. <late fusion> the two kind gate outputs. '''
	def __init__(self, opt):
		super(EncoderLstm_two_fc_nogate, self).__init__()
		torch.manual_seed(opt.seed)
		torch.cuda.manual_seed(opt.seed)
...
## POS Tag encoder ##
#####################
class Encode_Postag(nn.Module):
	def __init__(self, opt):
		super(Encode_Postag, self).__init__()
		self.category_size = opt.category_size
		self.input_encoding_size = opt.input_encoding_size
		self.rnn_size = opt.rnn_size

		self.embed = nn.Embedding(self.category_size, self.input_encoding_size)  # (20000,468)
		self.encoding = nn.LSTMCell(self.input_encoding_size, self.rnn_size)
	...
	def forward(self, postag, posmask):
		''' postag:(m, seq_len+1), posmask:(m, seq_len+1)
			posmask 除了后边统一是0，其他地方不全为1，1之间夹杂着0 '''
		batch_size, length = postag.size(0), postag.size(1)
		# make a 'real' mask
		mask = torch.zeros_like(posmask)
		for i in range(batch_size):
			index = np.argwhere(posmask[i, :] != 0)[0][-1]  # posmask_i 中最后一个不为0的地方
			mask[i, :index+1] = 1.0

		state = self.init_hidden(batch_size)
		for i in range(length):
			i_mask = mask[:,i].unsqueeze(-1)  # (m, 1)
			index = postag[:, i].clone()  # (m,)
			embedding = self.embed(index)  # (m, input_encoding_size)
			state_h, state_c = self.encoding(embedding, state)
			state_h = state_h * i_mask + state[0] * (1.0 - i_mask)
			state_c = state_c * i_mask + state[1] * (1.0 - i_mask)
			state = (state_h, state_c)
			postag_feat = state_h  # (
		postag_outputs = torch.stack(postag_outputs, dim=1)  # (m, seq_len+1, rnn_size)
		return postag_outputs
...
class LSTMCore_two_layer(nn.Module):
	...
	def forward(self, xt, xt_mask, V, state): # xt:(m,input_encoding_size) xt_mask:(m,1) V:(m,28,visual_size)
		...
		# tow-layer lstm
		output_1, state1 = self.lstm_1(xt, state1, xt_mask)
...
class LSTMCore_two_layer_posfeat(nn.Module):
	''' two-layer decoder:
		1. <soft attention> all input features;
		2. 双层LSTM, 第一层输入tokens和posfeats，第二层输入第一层的状态和attented特征'''
	def __init__(self,opt):
	# def __init__(self,input_encoding_size,rnn_size,drop_prob_lm,feat_size,att_size):
		super(LSTMCore_two_layer_posfeat,self).__init__()
		torch.manual_seed(opt.seed)
		torch.cuda.manual_seed(opt.seed)
		#  word and rnn size
		self.input_encoding_size = opt.input_encoding_size  #  468
