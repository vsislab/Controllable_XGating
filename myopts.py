...
def parse_opt():
    ...
    parser.add_argument('--data_path_pool', type=str, default='../datas/msrvtt_rgb_pool/',
                        help='the path contains another feats/,CAP.pkl and so on. ')
    parser.add_argument('--data_path_pool2', type=str, default='../datas/msrvtt_flow_pool/',
                        help='the path contains another feats/,CAP.pkl and so on. ')
    ...
    parser.add_argument('--feat_depth', type=int, default=1024, help='1024 is the depth of I3D pool features.(rgb)')
    parser.add_argument('--feat_depth2', type=int, default=1024, help='1024 is the depth of I3D pool features.(opfl)')
    parser.add_argument('--HxW', type=int, default=49, help='the height multply weight of the pooling features(rgb)')
    parser.add_argument('--HxW2', type=int, default=49, help='the height multply weight of the pooling features(opfl)')
    ...
    parser.add_argument('--max_epochs', type=int, default=60, help='number of epochs')
    ...
    # Recnet
    parser.add_argument('--rec_strategy', type=str, default=None, help = 'None/global/local/both, no-using or what type reconstructor')
    parser.add_argument('--weight_sa', type = float, default=1.0, help = 'trade-off parameters for sa model. Used when train with RecNet ')
    parser.add_argument('--weight_rec', type = float, default = 0.0)
    ...
    assert args.rec_strategy in [None, 'global', 'local', 'both'], "ckeck 'rec_strategy', should be None, 'global' or 'local' or 'both'"
    assert args.weight_sa >= 0, "SA model loss weight should not smaller than 0"
    assert args.weight_rec >= 0, "RecNet model loss weight should not smaller than 0"
