...
def train(opt):
	...
	if opt.load_best_score == 1:
		...
		best_val_score_meteor = infos.get('best_val_score_meteor', None)
		best_val_score_rouge = infos.get('best_val_score_rouge', None)
		best_val_score_bleu = infos.get('best_val_score_bleu', None)
	else:
		best_val_score = None
		best_val_score_meteor = None
		best_val_score_rouge = None
		best_val_score_bleu = None
		...
		for data, cap, cap_mask, cap_classes, class_mask, feat1, feat2, feat3, feat4, feat_mask, pos_feat, lens, groundtruth, image_id in myloader_train:
			...
			if opt.rec_strategy is None:
				if not sc_flag:
					...
					gen_result,sample_logprobs = model.sample(feat1, feat2, feat3, feat4,feat_mask, pos_feat, {'sample_max':0})
					reward = myutils.get_self_critical_reward(model,feat1, feat2, feat3, feat4, feat_mask, pos_feat, groundtruth,gen_result) # (m,max_length)
			...
			if (iteration % opt.save_checkpoint_every == 0):
				...
				if opt.language_eval == 1:
					current_score = lang_stats['ROUGE']
				...
				if opt.language_eval == 1:
					...
					# "rouge"
					current_score_rouge = lang_stats['CIDEr']
					best_flag = False
